...
namespace {
constexpr std::size_t kPort = 5001;
constexpr std::size_t kWidth = 1920;
constexpr std::size_t kHight = 1080;
}// namespace
  ...
  switch (event.type) {
    ...
    case ImGuiWS::Event::MouseMove: {
      if (event.clientId == curIdControl) {
        input_events.push_back(InputEvent{InputEvent::Type::kEMousePos,
                                         false,
                                         ImGuiKey_COUNT,
                                         -1,
    ...
    case ImGuiWS::Event::MouseDown: {
      if (event.clientId == curIdControl) {
        ...
        ImGuiMouseButton but_im_gui = event.mouse_but;
        switch (event.mouse_but) {
          case 1: but_im_gui = ImGuiMouseButton_Middle; break;
          case 2: but_im_gui = ImGuiMouseButton_Right; break;
          default: break;
        }

        input_events.push_back(InputEvent{InputEvent::Type::kEMouseButton,
                                         true,
                                         ImGuiKey_COUNT,
                                         but_im_gui,
                                         {event.mouse_x, event.mouse_y},
    ...
    case ImGuiWS::Event::MouseWheel: {
      if (event.clientId == curIdControl) {
        input_events.push_back(InputEvent{InputEvent::Type::kEMouseWheel,
                                         false,
                                         ImGuiKey_COUNT,
                                         -1,
                                         {},
    ...
    case ImGuiWS::Event::KeyUp: {
      if (event.clientId == curIdControl) {
        if (event.key > 0) {
          ImGuiKey key_im_gui = toImGuiKey(event.key);
          input_events.push_back(
              InputEvent{InputEvent::Type::kEKey, false, key_im_gui, -1, {}, 0.0F, 0.0F});
        }
      }
    } break;
    case ImGuiWS::Event::KeyDown: {
      if (event.clientId == curIdControl) {
        if (event.key > 0) {
          ImGuiKey key_im_gui = toImGuiKey(event.key);
          input_events.push_back(
              InputEvent{InputEvent::Type::kEKey, true, key_im_gui, -1, {}, 0.0F, 0.0F});
        }
      }
    } break;
    case ImGuiWS::Event::KeyPress: {
      if (event.clientId == curIdControl) {
        last_add_text.resize(1);
        last_add_text[0] = event.key;
      }
    } break;
    case ImGuiWS::Event::Resize: {
      std::cout << event.client_width << " " << event.client_height << '\n';

      ImGui::GetIO().DisplaySize = {static_cast<float>(event.client_width),
                                    static_cast<float>(event.client_height)};
    } break;
    default: {
...
void State::update() {
  ...
  if (clients.empty()) { curIdControl = -1; }

  if (curIdControl > 0) {
    {
      auto &io = ImGui::GetIO();

      if (!last_add_text.empty()) { io.AddInputCharactersUTF8(last_add_text.c_str()); }

      for (const auto &event : input_events) {
        switch (event.type) {
          case InputEvent::Type::kEKey: {
            io.AddKeyEvent(event.key, event.isDown);
          } break;
          case InputEvent::Type::kEMousePos: {
            io.AddMousePosEvent(event.mousePos.x, event.mousePos.y);
          } break;
          case InputEvent::Type::kEMouseButton: {
            io.AddMouseButtonEvent(event.mouseButton, event.isDown);
            io.AddMousePosEvent(event.mousePos.x, event.mousePos.y);
          } break;
          case InputEvent::Type::kEMouseWheel: {
...
void WebServer::Initialize() {
  imgui_ws_.init(kPort, "../static", {"", "index.html"});
  {
    unsigned char *pixels;
    int32_t width;
    int32_t height;
    ImGui::GetIO().Fonts->GetTexDataAsAlpha8(&pixels, &width, &height);
    imgui_ws_.setTexture(0, ImGuiWS::Texture::Type::Alpha8, width, height,
                         reinterpret_cast<const char *>(pixels));
  }
  ImGui::GetIO().DisplaySize = ImVec2(kWidth, kHight);
}

void WebServer::Tick() {
  auto events = imgui_ws_.takeEvents();
  for (auto &event : events) { state_.handle(std::move(event)); }
  state_.update();

  imgui_ws_.setDrawData(ImGui::GetDrawData());

  do { vsync_.wait(); } while (imgui_ws_.nConnected() == 0);
}
